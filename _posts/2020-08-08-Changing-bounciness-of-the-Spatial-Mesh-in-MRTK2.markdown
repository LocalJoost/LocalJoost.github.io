---
layout: post
title: Changing bounciness of the Spatial Mesh in MRTK2
date: 2020-08-08T00:00:00.0000000+02:00
tags: unity3d windows-mixed-reality hololens2 mrkt2
featuredImageUrl: https://LocalJoost.github.io/assets/2020-08-08-Changing-bounciness-of-the-Spatial-Mesh-in-MRTK2/SpatialNoPhysicalSetting.png
---
Long ago, when I made the [CubeBouncer for HoloLens 1](https://localjoost.github.io/hololens-cubebouncer-application-part-1/), I already noticed I could change the so called 'physical material' of holograms so they bounced off each other like bouncing balls. But as soon as those objects hit a floor, ceiling, wall or any other physical (i.e. the Spatial Mesh) they kind of stopped - like a bouncing ball hitting a carpet in stead of a stone floor. 

## 'Behind' the scenes
This is, of course, because a 'Hologram' is nothing more than a Unity game object, and so is the Spatial Mesh. What happens not-so-behind the scenes is the WindowsMixedRealitySpatialMeshObserver observing whatever SurfaceObserver is reporting from the HoloLens spatial mapping engine. It creates Mesh objects from that data, it adds a mesh collider, it applies a material - either the occlusion or the visibility material - and voila: all real objects are wrapped in a virtual objects, and the Unity engine seems to interacting with those real objects. This is how occlusion works, and how virtual objects seem to bounce off real objects - in fact they are stopped and/or hidden by the virtual 'wrap'.

## The missing piece
The mesh observers (there are in fact more than one) apply a material to the spatial meshes, but materials only determine the *display properties* of an object, not it's physical properties. And if you look at the Mixed Reality Toolkit Spatial Awareness settings
![](/assets/2020-08-08-Changing-bounciness-of-the-Spatial-Mesh-in-MRTK2/SpatialNoPhysicalSetting.png)  
...
A profile is a class that contains only settings. It's used to set properties of components and services in the MRTK2. For the SpatialMeshObservers, that's MixedRealitySpatialAwarenessMeshObserverProfile and it has quite a number of properties that can be set on a spatial mesh observer. We are simply going to add one At the end of the file you simply add a field and a property before the #endregion
```cs
[SerializeField]
...
![](/assets/2020-08-08-Changing-bounciness-of-the-Spatial-Mesh-in-MRTK2/PhysicsFieldAdded.png)
And you can simply create and drag a physical material on top of that. 
![](/assets/2020-08-08-Changing-bounciness-of-the-Spatial-Mesh-in-MRTK2/PhysicalMaterialAdd.png)
...
## The same effect *in* the Unity editor
...
In SpatialObjectMeshObserver you only have to find the method ApplyMeshMaterial, where you will find the exact same piece of code you found in WindowsMixedRealitySpatialMeshObserver and you will have to adapt that accordingly
...
You could also simply wait for [the pull request I made on the MRKT2 implementing this self safe functionality](https://github.com/microsoft/MixedRealityToolkit-Unity/pull/8264) to be accepted 
...
A little demo app implementing this proposed feature, including the modified MRTK2, can be found here