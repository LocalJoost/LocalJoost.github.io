...
---
I make extensive use of the MRTK2 extension service feature and it's dependency injection capabilities. Unfortunately it's not easy to see the internal state of such a service in the Unity editor while it's running, unless you are willing to write a custom inspector for it using the Unity Editor API. This API is, to put it mildly, not very intuitive. Writing editor UIs is a cumbersome and error-prone process. If your extension service grows and holds more complex data, maintaining it becomes a chore as for *every single field* you have to add UI code. 
